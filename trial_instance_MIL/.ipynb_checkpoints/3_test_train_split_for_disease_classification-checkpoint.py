...
df = pd.read_csv(root_dir / '16tb2/ALL_Subtype_Classification/merged_subtypes.csv')
...
def create_cohort(cls_list, included_df, all_group):
    s_list = included_df.loc[included_df['assistant_IHC'] == cls_list]['Image_ID'].to_list()
    s_list += included_df.loc[included_df['hrchen_IHC'] == cls_list]['Image_ID'].to_list()
...
for cls_list in ['B-ALL', 'T-ALL']:
    create_cohort(cls_list, included_df, all_group)
assert set(check_duplicate['B-ALL']) & set(check_duplicate['T-ALL']) == set()

# In[27]:
# store in json format for cohort copying

with open(root_dir / '16tb2/ALL_Subtype_Classification/trial_instance_MIL_cohort.json', 'w') as f:
